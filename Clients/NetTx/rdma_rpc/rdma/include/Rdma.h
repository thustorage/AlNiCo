...
// <128 qp size
#define BatchPostRec 32ULL

#define BatchPollSend 32ULL
#define BatchSignalSend 32ULL

#define MAX_POST_LIST 32
#define DCT_ACCESS_KEY 0x0000000000001234
...
struct UD_DC_context_t{  
    ibv_exp_dct *dct_tagert; // for server.
    peer_data_t **peer_data;

    UD_DC_context_t(){

    }
    UD_DC_context_t(int server_num){
        peer_data = (peer_data_t **)malloc(sizeof(peer_data_t*) * server_num);
    }
    void malloc_region_info_in_client(int server_id, int region_num){
        peer_data[server_id] = (peer_data_t *)malloc(sizeof(peer_data_t) * region_num);
        for (int i = 0; i < region_num; i++){
            peer_data[server_id][i].online = false;
        }
        return;
    }
    void malloc_thread_info_in_server(int client_id, int thread_num){
        peer_data[client_id] = (peer_data_t *)malloc(sizeof(peer_data_t) * thread_num);
        return;
    }   
...
static inline void fillSgeWr_1024(ibv_sge* sg, ibv_send_wr& wr, uint64_t source,
    uint64_t size, uint32_t lkey)
{
    int i;
    for (i = 0; i < (size+1023)/1024; i++){
        sg[i].addr = (uintptr_t)source + i * 1024;
        sg[i].length = 1024;
        sg[i].lkey = lkey;
    }
    memset(&wr, 0, sizeof(wr));
