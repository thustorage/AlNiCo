...
uint64_t test1;
uint64_t test1_array[8] = {32, 64, 128, 256, 512, 1024, 2048, 4096};
// uint64_t test1_array[4] = {256, 1024, 4096};
...
double check()
{
    ...
    std::set<pair<uint64_t, uint64_t>> no_contention_0;
    std::set<pair<uint64_t, uint64_t>> no_contention_1;
    uint8_t inter_f[512];

    result.clear();
    no_contention_0.clear();
    no_contention_1.clear();
    int tmp_total_key = 0;
    int tmp_bad_key = 0;
    ...
    while (it != txs[0].end())
    {
        ...
        {
            no_contention_0.insert(make_pair((*it).first, (*it).second));
            tmp_total_key++;
        }
        it++;
    }
    ...
    while (it != txs[1].end())
    {
        if (txs[0].find(make_pair((*it).first, (*it).second)) == txs[0].end())
        {
            no_contention_1.insert(make_pair((*it).first, (*it).second));
            tmp_total_key++;
        }
        it++;
    }
    ...
    {
        bool real = ((inter_f[i / 8] & (1U << (i % 8))) != 0);                            // contention in tx
        bool fake = ((txfeature[0][i / 8] & (1U << (i % 8)) & txfeature[1][i / 8]) != 0); // contention in feature
    ...
    // count tx0
    it = no_contention_0.begin();
    while (it != no_contention_0.end())
    {
        uint64_t feature_offset = hash_k2f_2(*it); // no contention in tx
        // printf("%d %d\n", it->first, it->second);
        // bool real = ((inter_f[i / 8] & (1U << (i % 8))) != 0);
        if ((txfeature[1][feature_offset / 8] & (1U << (feature_offset % 8))) != 0){
            tmp_bad_key++;
        }
        it++;
    }

    it = no_contention_1.begin();
    while (it != no_contention_1.end())
    {
        uint64_t feature_offset = hash_k2f_2(*it); // no contention in tx
        if ((txfeature[0][feature_offset / 8] & (1U << (feature_offset % 8))) != 0){
            tmp_bad_key++;
        }
        it++;
    }
...
int main()
{
    for (int x = 0; x < 8; x++)
    {
        ...
        dd = new sampling::StoZipfDistribution<>(rng, 0, YCSB_TABLE_SIZE - 1, 1.1);
        ...
        printf("ans(zipf=%d)=%lf %lf\n", test1, (shit_real + 0.001) / shit_ans, (total_key - bad_key) * 1.0 / total_key);
    }
    return 0;
}