...
struct NodeInfo {
    ...
    uint32_t message_size_rpc_reply; // nictxn's single message, switchtx's buf
...
struct alignas(64) Connect {
    ...
    void fill_wr(uint32_t lkey) // TODO:
    {
    ...
    inline uint64_t get_remote_reply_addr_by_id(uint16_t id)
    {
        return remote_reply_addr + id * REPLY_LEN;
    }

    inline uint64_t get_remote_reply_addr_by_id_strife(uint32_t rpc_id)
    {
        return remote_reply_addr + REPLY_LEN * SLOT_NUM + rpc_id * 8;
    }

    inline uint64_t get_strife_send_addr(uint32_t rpc_id) volatile
    {
        return reply_addr + REPLY_LEN * SLOT_NUM + rpc_id * 8;
    }

    inline uint32_t get_strife_reply_size(){
        return 8 * IDMAPADDRSIZE;
    }
    ...
    inline uint64_t serialization(uint16_t id, uint64_t* size, uint64_t feature_addr)
    {
        ...
        uint64_t data_addr = localBase + 2 * poolSize + id * DATA_LEN;
        ReqHead* reqHead = (ReqHead*)(data_addr);
        *size = 4 + 64;
