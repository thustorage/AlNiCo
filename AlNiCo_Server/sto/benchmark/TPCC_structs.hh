...
struct warehouse_key {
    ...
    tx_key_t nic_key(){
        return make_pair(WARE, bswap(w_id));
    }
    tx_key_t nic_father_key(){
        return make_pair(NTABLE, 0);
    }
...
{
    uint32_t did = d_id + (w_id * NUM_DISTRICTS_PER_WAREHOUSE);
...
{
    uint32_t upper_id = w_id * NUM_DISTRICTS_PER_WAREHOUSE + d_id;
...
{
    uint32_t cid = (h_c_w_id * NUM_DISTRICTS_PER_WAREHOUSE + h_c_d_id) * 3000 + h_c_id;
    uint32_t did = h_d_id + (h_w_id * NUM_DISTRICTS_PER_WAREHOUSE);
...
{
    uint32_t upper_id = (w_id * NUM_DISTRICTS_PER_WAREHOUSE + d_id) * 3000 + c_id;
    uint64_t id = static_cast<uint64_t>(upper_id) << 32 | static_cast<uint64_t>(o_id);
    return id;
}
...
struct district_key {
    ...
    tx_key_t nic_key(){
        // printf("DIST, %lld %lld d_w_id=%d d_id=%d\n", DIST, makeDistrictKey(d_w_id, d_id), d_w_id, d_id);
        return make_pair(DIST, makeDistrictKey(bswap(d_w_id), bswap(d_id)));
    }
    // FIX
    tx_key_t nic_father_key(){
        return make_pair(WARE, bswap(d_w_id));
    }
...
struct customer_idx_key {
    ...
    tx_key_t nic_key(){
        return make_pair(CUST, makeDistrictKey(bswap(c_w_id), bswap(c_d_id)));
    }
...
struct customer_key {
    ...
    tx_key_t nic_key(){
        return make_pair(CUST, makeCustomerKey(bswap(c_w_id), bswap(c_d_id), bswap(c_id)));
    }
    tx_key_t nic_father_key(){
        // printf("DIST, %lld %lld d_w_id=%d d_id=%d\n", DIST, makeDistrictKey(d_w_id, d_id), d_w_id, d_id);
        return make_pair(DIST, makeDistrictKey(bswap(c_w_id), bswap(c_d_id)));
    }
...
struct order_key {
    ...
    tx_key_t nic_key(){
        return make_pair(ORDE, makeOrderKey(bswap(o_w_id), bswap(o_d_id), bswap(o_id)));
    }
    tx_key_t nic_father_key(){
        return make_pair(DIST, makeDistrictKey(bswap(o_w_id), bswap(o_d_id)));
    }
...
struct orderline_key {
    ...
    tx_key_t nic_key(){
        return make_pair(ORLI, makeOrderLineKey(bswap(ol_w_id), bswap(ol_d_id), bswap(ol_o_id), bswap(ol_number)));
    }
    tx_key_t nic_father_key(){
        return make_pair(ORDE, makeOrderKey(bswap(ol_w_id), bswap(ol_d_id), bswap(ol_o_id)));
    }
...
struct item_key {
    ...
    tx_key_t nic_key(){
        return make_pair(ITEM, bswap(i_id));
    }
    tx_key_t nic_father_key(){
        return make_pair(NTABLE, 0);
    }
...
struct stock_key {
    ...
    tx_key_t nic_key(){
        return make_pair(STOC, makeStockKey(bswap(s_w_id), bswap(s_i_id)));
        // return make_pair(STOC, makeStockKey(bswap(s_w_id), bswap(s_i_id)));
    }
    tx_key_t nic_father_key(){
        return make_pair(WARE, bswap(s_w_id));
    }
