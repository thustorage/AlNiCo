...
struct SplitParams<tpcc::district_value> {
  ...
  static constexpr auto split_builder = std::make_tuple(
    [](const tpcc::district_value& in) -> tpcc::district_value {
      ...
      out.d_tax = in.d_tax;
      out.d_ytd = in.d_ytd;
      out.d_next_o_id = in.d_next_o_id;
      return out;
    }
  );
...
class RecordAccessor<A, tpcc::district_value> {
 public:
  ...
  const uint64_t& d_next_o_id() const {
    return impl().d_next_o_id_impl();
  }
...
class UniRecordAccessor<tpcc::district_value> : public RecordAccessor<UniRecordAccessor<tpcc::district_value>, tpcc::district_value> {
 ...
 private:
  ...
  const uint64_t& d_next_o_id_impl() const {
    return vptr_->d_next_o_id;
  }
  ...
  void copy_into_impl(tpcc::district_value* dst) const {
    ...
    if (vptr_) {
      ...
      dst->d_next_o_id = vptr_->d_next_o_id;
    }
  }
 ...
 private:
  ...
  void copy_into_impl(tpcc::district_value* dst) const {
    ...
    if (vptr_0_) {
      ...
      dst->d_next_o_id = vptr_0_->d_next_o_id;
    }
