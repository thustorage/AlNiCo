...
// work_msg->abort_key(key.nic_father_key());

#define RECORED_ABORT(abort, key) if (!abort) {\
        work_msg->abort_key(key.nic_key());\
        }
#define TPCC_RETRY_COUNT(abort, id) if (!abort) {\
...
void tpcc_runner<DBParams>::run_txn_neworder(int did, worker_msg_t *work_msg) {
    ...
    // uint64_t succ_w_id, succ_d_id, succ_o_id;
    // OLD STO
#ifdef QUICKCHECK
    uint64_t q_w_id  = ig.random(w_id_start, w_id_end);
    // uint64_t q_w_id  = ig.random(1, 2);
    uint64_t q_d_id = ig.random(1, 10);
    // q_d_id = did; // quick check
    ...
    uint64_t q_w_id  = work_msg->now_msg.get_w_id();
    uint64_t q_d_id = work_msg->now_msg.get_d_id();
    uint64_t q_c_id = work_msg->now_msg.get_c_id();
    assert(q_c_id != 0);

    uint64_t num_items = work_msg->now_msg.get_s_num();
    //uint64_t rbk = ig.random(1, 100); //XXX no rollbacks
    // printf("w d c n %d %d %d %d\n", q_w_id, q_d_id, q_c_id, num_items);
    uint64_t ol_i_ids[15];
    uint64_t ol_supply_w_ids[15];
    uint64_t ol_quantities[15];
    ...
    bool all_local = true;
    for (uint64_t i = 0; i < num_items; ++i) {
        uint64_t ol_i_id = work_msg->now_msg.get_i_id(i);
        ol_i_ids[i] = ol_i_id;
        ol_supply_w_ids[i] = work_msg->now_msg.get_s_w(num_items, i);
        // printf("i s s_w %d %d\n", i, ol_i_id, ol_supply_w_ids[i]);

        bool supply_from_remote = (q_w_id != ol_supply_w_ids[i]);
        if (supply_from_remote) {
            all_local = false;
        }
    ...
    RWTXN {
    if (starts!=0){
        TPCC_RETRY_COUNT(false,runner_id);
    }
    ++starts;
    ...
    RECORED_ABORT(abort, wk); // FUCK: abort is success
    CHK(abort);
    assert(result);
    ...
    auto [abort, result, row, value] = db.tbl_districts(q_w_id).select_split_row(dk,
        {{dt_nc::d_tax, access_t::read},
        {dt_nc::d_next_o_id, access_t::update}}

    );
    // if (abort){
    //     printf("id1 id2 = %llx %llx %llx %llx %llx\n",q_w_id, q_d_id, bswap(q_w_id), bswap(q_d_id), bswap(bswap(q_d_id)));
    // }
    (void)row; (void)result;
    RECORED_ABORT(abort, dk);
    RECORED_ABORT(false, dk);
    CHK(abort);
    assert(result);
    ...
    new_dv->d_next_o_id = dt_next_oid;
    // succ_w_id = q_w_id;
    // succ_d_id = q_d_id;
    // succ_o_id = dt_next_oid;
    ...
    for (uint64_t i = 0; i < num_items; ++i) {
        ...
        (void)row; (void)result;
        RECORED_ABORT(abort, item_key(iid));
        CHK(abort);
        assert(result);
        oid = value.i_im_id();
        CHK(oid != 0);
        ...
        (void)result;
        RECORED_ABORT(abort, stock_key(wid, iid));
        // RECORED_ABORT(false, stock_key(wid, iid));
        CHK(abort);
        assert(result);
    ...
    // order_key ok(succ_w_id, succ_d_id, succ_o_id);
    // // uint64_t q_c_id = 0;
    // uint32_t ol_cnt = 0;
    // {
    // auto [abort, result, row, value] = db.tbl_orders(q_w_id).select_split_row(ok,
    ...
    // if (row == 0){  
    //     printf("fuck %d %d %d %d\n", succ_w_id, succ_d_id, succ_o_id, q_c_id);
    //     exit(0);   
    // }
    // }


    // printf("new order %d %d %d\n", succ_w_id, succ_d_id, succ_o_id);
...
/*TODO:payment*/
template <typename DBParams>
void tpcc_runner<DBParams>::run_txn_payment(int did, worker_msg_t *work_msg) {
    typedef warehouse_value::NamedColumn wh_nc;
    typedef district_value::NamedColumn dt_nc;
    typedef customer_value::NamedColumn cu_nc;
...
#else
    uint64_t q_w_id = work_msg->now_msg.get_w_id_pay();
    uint64_t q_d_id = work_msg->now_msg.get_d_id_pay();

    uint64_t q_c_w_id, q_c_d_id, q_c_id;
    std::string last_name;

    bool is_home = work_msg->now_msg.get_is_home_pay();
    bool by_name = work_msg->now_msg.get_by_name_pay();

    if (is_home) {
        q_c_w_id = q_w_id;
        q_c_d_id = q_d_id;
    } else {
        q_c_w_id = work_msg->now_msg.get_c_w_id_pay();
        q_c_d_id = work_msg->now_msg.get_c_d_id_pay();
    }

    if (by_name) {
        last_name = ig.to_last_name(work_msg->now_msg.get_c_id_pay());
        q_c_id = 0;
    } else {
        q_c_id = work_msg->now_msg.get_c_id_pay();
    }
...
/*TODO:orderstatus*/
template <typename DBParams>
void tpcc_runner<DBParams>::run_txn_orderstatus(int did, worker_msg_t *work_msg) {
    typedef customer_value::NamedColumn cu_nc;
    typedef order_value::NamedColumn od_nc;
    typedef orderline_value::NamedColumn ol_nc;
    std::string last_name;

    uint64_t q_w_id = work_msg->now_msg.get_w_id_order();
    uint64_t q_d_id = work_msg->now_msg.get_d_id_order();
    uint64_t q_c_id;
    bool by_name = work_msg->now_msg.get_by_name_order();
    if (by_name) {
        last_name = ig.gen_customer_last_name_run();
        q_c_id = 0;
    } else {
        q_c_id = work_msg->now_msg.get_c_id_order();
    }
...
void tpcc_runner<DBParams>::run_txn_delivery(uint64_t q_w_id,
    ...
    for (uint64_t q_d_id = 1; q_d_id <= 10; ++q_d_id) {
        ...
        // order_id = last_delivered[q_d_id - 1] + 1;
        ...
        // delivered_order_ids[q_d_id - 1] = last_delivered[q_d_id - 1] + 1;
        if (order_id == 0)
            continue;

        order_key ok(q_w_id, q_d_id, order_id);
        uint64_t q_c_id = 0;
        // printf("read %d,%d,%d q_c_id = %d \n", q_w_id, q_d_id, order_id, q_c_id);
        uint32_t ol_cnt = 0;
        {
        auto [success, result, row, value] = db.tbl_orders(q_w_id).select_split_row(ok,
            {{od_nc::o_c_id, access_t::read},
        ...
        if (row == 0){
            ...
            //         .template range_scan<decltype(no_scan_callback), false/*reverse*/>(k0, k1, no_scan_callback, RowAccess::ObserveValue, true, 1);
            // CHK(scan_success);
            // printf("fuck readfail %d,%d,%d \n", q_w_id, q_d_id, order_id); 
            // exit(0);           
            // delivered_order_ids[q_d_id - 1] = last_delivered[q_d_id - 1] + 1;
            continue;
        }
        ...
        // printf("readok %d,%d,%d q_c_id = %d (%d,%d)\n", q_w_id, q_d_id, order_id, q_c_id, success, result);
        ...
        if constexpr (Commute) {
            ...
            ol_cnt = new_ov->o_ol_cnt;
            q_c_id = new_ov->o_c_id;
        ...
        for (uint32_t ol_num = 1; ol_num <= ol_cnt; ++ol_num) {
            ...
            if constexpr (Commute) {
                ...
                ol_amount_sum += new_olv->ol_amount;
...
// TODO:txn_stocklevel
template <typename DBParams>
void tpcc_runner<DBParams>::run_txn_stocklevel(int did, worker_msg_t *work_msg){
    typedef orderline_value::NamedColumn ol_nc;
    typedef stock_value::NamedColumn st_nc;
    typedef district_value::NamedColumn dt_nc;


    uint64_t q_w_id = work_msg->now_msg.get_w_id_stock();
    uint64_t q_d_id = work_msg->now_msg.get_d_id_stock();
    // uint64_t q_w_id = ig.random(w_id_start, w_id_end);
    // uint64_t q_d_id = ig.random(1, 10);
    ...
    district_key dk(q_w_id, q_d_id);
    auto [success, result, row, value] = db.tbl_districts(q_w_id).select_split_row(dk,
        {{dt_nc::d_next_o_id, access_t::read}}

    );
    
    (void)row; (void)result;
    RECORED_ABORT(success, dk);
    CHK(success);
    assert(result);
    // d_next_oid = new_dv->d_next_o_id;

    d_next_oid = value.d_next_o_id();
    }
    // printf("id %d %d = %d\n", q_w_id, q_d_id, d_next_oid);
    // puts("?? stock level");
    // return;
