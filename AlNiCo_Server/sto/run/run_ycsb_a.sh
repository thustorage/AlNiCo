...
run() {
  ...
  do
    ...
    if [ $IS_MVCC -gt 0 ]
    then
      printf "Running 2PL on $BINARY $HEADER_LABEL\n"
      run_bench $OUTFILE $DELIVERY_OUTFILE $BINARY "$CT_FLAGS" "$HEADER_LABEL" $ITERS $THREADS "${P2PL_LABELS[@]}"
    else
...
rm -rf results
mkdir results
RFILE=results/ycsb_a_results.txt
DFILE=results/ycsb_a_delivery_results.txt
